//
//	Title: tictactoe_c.c
//	Purpose: Tictactoe game allowing the user to replay.
...
void generate_player2_move(char a[][3]);
...
do{
	...
	if(done == 'O'){
		...
		printf("Would you like to play again? (Y/N): "); // asks if the user wants to play again
		scanf("%c", &response);
		scanf("%c", &response);
	...
	if(response == 'N' || response == 'n'){ 
		...
		answer = false; // if false then program ends

	}else{

		answer = true; // if true then loop program again.
	}
	done = '\0'; // resets the value of "done" so that game can repeat.
}while(answer == true);
return 0;
}
...
void generate_player1_move(char a[][3], int row, int col){
	...
	if(!check_legal_option(a, row, col)){
		printf("Invalid move, please try again. "); // checks if the user's input is valid.
